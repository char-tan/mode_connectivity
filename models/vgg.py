...
class VGG(nn.Module):
    ...
    def _permutation_spec(self):
        ...
        return permutation_spec_from_axes_to_perm({
            ...
            # conv layers
            **{f"features.{is_conv[i]}.weight": (f"P_Conv_{is_conv[i]}", f"P_Conv_{follows_conv[i]}", None, None,)
                for i in range(len(is_conv))},
            **{f"features.{i}.bias": (f"P_Conv_{i}",) for i in is_conv + [0]},

            # layer norms
            **{f"features.{is_norm[i]}.weight": (f"P_Conv_{is_conv[i]}", None) for i in range(len(is_norm))},
            **{f"features.{is_norm[i]}.bias": (f"P_Conv_{is_conv[i]}", None) for i in range(len(is_norm))},
